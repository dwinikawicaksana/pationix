...
export default function FooterNew({
  ...
  const handleSubmit = () => {
    ...
    const subject =
      language === "id" ? "Pesanan Demo Paitonix" : "Paitonix Demo Request";
    const body =
      language === "id"
        ? `Halo, saya ingin konsultasi dalam pembuatan jasa website.%0A%0AEmail saya: ${encodeURIComponent(email)}`
        : `Hello, I would like to consult about website services.%0A%0AMy email: ${encodeURIComponent(email)}`;

    window.location.href = `mailto:hello@paitonix.com?subject=${encodeURIComponent(subject)}&body=${body}`;
    setShowSuccess(true);
    setEmail("");
  };
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr_1.2fr] gap-16 mb-16 items-start">
          ...
          >
            ...
            <div className="mt-6 text-sm text-slate-300 dark:text-gray-400 space-y-2 max-w-xs">
              <p>
        ...
        >
          ...
          <div className="flex items-center gap-6">
            <button
              type="button"
              className="group relative inline-flex h-10 w-10 items-center justify-center rounded-full bg-slate-800/80 text-slate-100 backdrop-blur-xl border border-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.2)] cursor-not-allowed transition-all duration-300"
            ...
            >
              ...
              <span className="pointer-events-none absolute -bottom-7 left-1/2 -translate-x-1/2 rounded-full bg-slate-900 px-2 py-1 text-[10px] uppercase tracking-[0.18em] text-slate-100 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                Coming soon
              </span>
            </button>
            <button
